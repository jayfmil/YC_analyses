function YC2_makePreStimReport(subjs,params,overwrite)
...
dataDir = fullfile(params.basePath,f(params.useCorrectedPower,y),'YC2_preStim');
...
for s = 1:length(subjs)
    subj  = subjs{s};
    fname = fullfile(dataDir,[subj '_YC2_preStimChange.mat']);
    if ~exist(fname,'file')
        continue
    end
...
mtl  = ~cellfun('isempty',regexpi(region,['amy|phc|prc|BA36']))';
oth  = ~(ec | hipp | mtl);
keyboard
...
subjFilter = {@(x) (~isnan(x) & ~isnan(deltaRR)),@(x) (~isnan(x) & ~isnan(deltaRR) & yc1Score>.9),@(x) (~isnan(x) &  numNZweights>5)};
subjFilter = {@(x) (~isnan(x) & ~isnan(deltaRR) &~oth),@(x) (~isnan(x) & ~isnan(deltaRR) & yc1Score>.95 &~oth),@(x) (~isnan(x) &  numNZweights>5 &~oth)};

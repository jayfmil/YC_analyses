...
try
    ...
    for iFold = 1:nFolds
        ...
        [res.yProb{iFold},...
            ...
            res.Cs{iFold},...
            res.Ts{iFold},...
            res.aucs{iFold}] = doRegFun(X,Y,T,folds,iFold,lambda,normType,sessInds,prctileThresh,nestedCvGroup,params.percentCV,useKfold,Cs,tBest,doCalcPenalty,doPermute);
...
function [cBest,tBest,Cs,Ts,auc_pen] = calcPenalty(X,Y,T,folds,sessInds,normType,prctileThresh,Cs)
...
function [yProbs,yPreds,yTest,A,err,lambda,tBest,Cs,Ts,aucs] = doRegFun(X,Y,T,folds,iFold,lambda,normType,sessInds,prctileThresh,nestedCvGroup,percentCV,useKfold,Cs,tBest,doCalcPenalty,doPermute)
...
if doCalcPenalty
    ...
    [lambda,tBest,Cs,Ts,aucs] = calcPenalty(xTrain,yTrainBool,T,subFolds,sessions,normType,prctileThresh,Cs);
else
    Ts = tBest;
    aucs = NaN;
end

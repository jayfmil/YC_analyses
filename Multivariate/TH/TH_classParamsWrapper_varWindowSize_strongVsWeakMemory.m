function TH_classParamsWrapper_varWindowSize_strongVsWeakMemory(subjs,just_loso)
%
...
% base save directory
baseDir = '/scratch/jfm2/TH1/multi/gridsearch_varWindowSize_strongVsWeak'
if ~doBipol
...
for s = 1:length(subjs)
    ...
    if exist(subjFile,'file')
        ...
        % check to make sure subject is doing the confidence judgements
        % correctly. Only run if so. 
        % - performance for high conf must be better
        % - must have enough high-conf rec and low-conf non-rec
        %   - percent correct must be between 10 and 90.
        %   - at least 10 in each class
        events = get_sub_events('RAM_TH1',subjs{s});
        encEvents   = events(strcmp({events.type},'CHEST') & [events.confidence]>=0);
        confs       = [encEvents.confidence]';
        highConf    = confs == 2;        
        rec         = [encEvents.recalled]';
        recByConf   = grpstats(rec,highConf);
        newRec      = rec((rec==1 & highConf) | (rec==0 & ~highConf));
        
        if length(recByConf) < 2;continue;end
        
        aucsSubj = NaN(nWindows,nBinCenters,nFreqs,nRegions);
        if (recByConf(2) > recByConf(1)) && (mean(newRec) > .1 && mean(newRec) < .9) && (sum(newRec) >= 10 && sum(newRec==0) >= 10)            
            parfor ind = 1:totalCalcs
                ...
                if ~isempty(timeWin)
                    ...
                    params.strongRecVsWeak = 1;
                    %                 params.classifyConf = 1;
...
texName = 'auc_triangle_strong_vs_weak_bipol';
if doBipol == 0
    texName = 'auc_triangle_strong_vs_weak_avg_ref';
end
if just_loso

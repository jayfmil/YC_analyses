function [aucAll,psAll] = TH_plotClassRes(subjs,params,overwrite,calcP)
...
% calculate p-value for auc?
if ~exist('calcP','var') || isempty(calcP)
    calcP = false;
end
...
for s = 1:length(subjs)
    ...
    if calcP
        ...
        classParams = classData.params;
        classParams.doPermute = 1;
        classParams.savePower = 1;
        classParams.loadPower = 0;
        classParams.saveOutput = 0;        
        permAucs(1) = TH1_refactor_phase(subj,classParams,classParams.basePath);            
        
        parfor iter = 2:size(permAucs,2)
            classParams = classData.params;
            classParams.doPermute = 1;
            classParams.savePower = 0;
            classParams.loadPower = 1;
            classParams.saveOutput = 0;            
            permAucs(iter) = TH1_refactor_phase(subj,classParams,classParams.basePath);
        end
        psAll(s) = nanmean(permAucs > classData.res.auc);
    end
    ...
    if ~exist(fname,'file') || overwrite
        ...
        figure(2)
        clf
        labels = vertcat(classData.res.yTests{:});
        labelsAll{end+1} = labels;        
        scores = vertcat(classData.res.yProbs{:});
        scoresAll{end+1} = scores;
        ...
        set(gca,'ytick',0:.2:1)
        set(gca,'fontsize',20);
        acc = mean(vertcat(classData.res.yPreds{:}) == vertcat(classData.res.yTests{:}));
        titleStr = sprintf('AUC = %.3f, p = %.3f, acc = %.2f',classData.res.auc,psAll(s),acc);
        title(titleStr)
        print('-depsc2','-tiff','-loose',fname);       
    end
    figs = [figs;figs_subj];        

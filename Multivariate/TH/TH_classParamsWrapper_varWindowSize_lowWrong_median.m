function TH_classParamsWrapper_varWindowSize_lowWrong_median(subjs,just_loso)
%
...
% base save directory
baseDir = '/scratch/jfm2/TH1/multi/gridsearch_varWindowSize_lowWrong_median'
if ~doBipol
...
for s = 1:length(subjs)
    ...
    % load events and figure out threshold for correct based on median of
    % non-low conf responses    
    ...
    enc = events(strcmp({events.type},'CHEST') & [events.confidence]>0);
    correctThresh = max([enc(1).radius_size median([enc.distErr])]);
...
%                 params.usePhase = 3
                ...
                params.correctThresh = correctThresh;
...
texName = 'auc_triangle_lowWrong_median_bipol';
if doBipol == 0
    texName = 'auc_triangle_lowWrong_median_avg_ref';
end
if just_loso

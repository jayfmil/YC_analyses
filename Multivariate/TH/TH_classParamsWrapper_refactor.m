function TH_classParamsWrapper_refactor(subjs)
%
...
% base save directory
baseDir = '/scratch/jfm2/TH1/multi/gridsearch_500ms_slidingWindow'
subDir  = fullfile(baseDir,'t_%d_c_%d_f_%d_r_%d');

% possible parameters
timeBins = {[26 50],...  %      -500 - 0 ms
    [51 75],...  %       0 - 500 ms
    [76 100],... %    500 - 1000 ms
    [101 125],...%   1000 - 1500 ms
    [26 75],...  %    -500 - 500 ms
    [51 100],... %      0 - 1000 ms
    [51 125],... %      0 - 1500 ms
    [26 125]};   %   -500 - 1500 ms

% 500 ms window from -1900 - 2000, sliding 100 ms
timeBins=num2cell([[5:5:175]' [30:5:200]'],2);

Cs         = logspace(log10(1e-6),log10(1e4),22);
powerPaths = {'/scratch/jfm2/power8freqs','/scratch/jfm2/power4bins_hilbert','/scratch/jfm2/power50freqs'};
regions    = {'all','mtl'};
...
%             params.excludeBadChansWatrous = 1;
            ...
            if exist(fullfile(saveDir,[subj '_class_pow.mat']),'file')
                subjDataTmp = load(fullfile(saveDir,[subj '_class_pow.mat']));
                aucsSubj(ind) = subjDataTmp.res.auc;                
            else
...
% region
fun = @(x) [num2str((x(1)-100)*20),'-',num2str((x(2)-100)*20)];
timeBins_str = cellfun(fun,timeBins,'Uniformoutput',false);
figs = [];
count = 1;
freqLabels = {'8 Frequencies','4 bands','50 Frequencies'};
...
for f = 1:nFreqs
    for r = 1:nRegions
        ...
        aucsBestC = NaN(size(aucs,1),size(aucs,3));
        bestC     = NaN(size(aucs,1),size(aucs,3));      
        for s = 1:size(aucsBestC,2);
           otherSubs = setdiff(1:size(aucsBestC,2),s);
           for t = 1:nTimes
               [~,cInd] = max(nanmean(squeeze(aucs(t,:,otherSubs)),2));
               aucsBestC(t,s) = aucs(t,cInd,s);
               bestC(t,s) = Cs(cInd);
           end
        end
        ...
        figure(3)
        clf
        e = nanstd(squeeze(aucs(:,7,:)),[],2) ./ sqrt(sum(~isnan(squeeze(aucs(1,1,:))))-1) * 1.96;        
        h=plot(nanmean(squeeze(aucs(:,7,:)),2),'linewidth',3);
        hold on
        h2=plot(nanmean(squeeze(aucs(:,7,:)),2)+e,'--','linewidth',3,'Color',h.Color);
        h3=plot(nanmean(squeeze(aucs(:,7,:)),2)-e,'--','linewidth',3,'Color',h.Color);
        set(gca,'xtick',1:nTimes)
        set(gca,'xlim',[0 nTimes + 1]);
        set(gca,'xticklabel',timeBins_str);
        grid on
        set(gca,'gridlinestyle',':');
        set(gca,'XTickLabelRotation',45)
        xlabel('Time Bin (ms)','fontsize',20)
        ylabel('AUC','fontsize',20)
        set(gca,'fontsize',16)
        fname = fullfile(figDir,sprintf('aucTime_standard_f%d_r%d.eps',f,r));
        print('-depsc2',fname);
        figs(count).aucTimeStandard = fname;          
        
        figure(4)
        clf
        e = nanstd(aucsBestC,[],2) ./ sqrt(sum(~isnan(aucsBestC(1,:)))-1) * 1.96;
        h=plot(nanmean(aucsBestC,2),'linewidth',3);
        hold on
        h2=plot(nanmean(aucsBestC,2)+e,'--','linewidth',3,'Color',h.Color);
        h3=plot(nanmean(aucsBestC,2)-e,'--','linewidth',3,'Color',h.Color);
        set(gca,'xtick',1:nTimes)
        set(gca,'xlim',[0 nTimes + 1]);
        set(gca,'xticklabel',timeBins_str);
        grid on
        ...
        ylabel('AUC','fontsize',20)
        set(gca,'fontsize',16) 
        fname = fullfile(figDir,sprintf('aucTime_best_f%d_r%d.eps',f,r));
        print('-depsc2',fname);
        figs(count).aucTimeBest = fname;          

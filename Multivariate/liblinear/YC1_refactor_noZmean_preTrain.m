function [perf,AUC,subject,params,res] = YC1_refactor_noZmean_preTrain(subj,params,saveDir)
...
try
    ...
    % take certain perentage of the data to optimize the hyperparameters
    trainPerSess = round(grpstats(sessions,sessions,'numel')*.33);
    trainInds    = false(size(powMat,1),1);
    uniqSess = unique(sessions);
    for sess = 1:length(uniqSess)
        sessInds  = randsample(find(sessions == uniqSess(sess)),trainPerSess(sess));
        trainInds(sessInds) = true;
    end
    
    % get hyperparamter training data
    [~,subFolds] = createFolds(sessions(trainInds),sel(trainInds));
    xTrain    = powMat(trainInds,:);
    yTrain    = Y(trainInds);
    sessTrain = sessions(trainInds);
    
    % run to find optimal enc, time bin, and penalty
    aucs = run_nested_fold(xTrain,encMat,timeMat,yTrain,sessTrain,subFolds,params,powParams);
    thresh = prctile(aucs(:),95);
%     ind=find(aucs==max(aucs(:)),1,'first');
    ind=find(aucs>=thresh,1,'first');
    ...
    thisT = Ts(tBestInd);
    thisC = Cs(cBestInd);
    fprintf('Max AUC = %.3f, chosen AUC = %.3f, best enc = %d, best time = %d, best C = %.4f\n',max(aucs(:)),thresh,thisE,thisT,thisC)
    
    
    % remove those trials from rest of analysis
    powMat   = powMat(~trainInds,:);
    Y        = Y(~trainInds);
    sessions = sessions(~trainInds);
    sel      = sel(~trainInds);
    ...
    for thisFold = 1:nFolds
        ...
        [yProbFold,yTestFold] = run_fold_with_nested_folds(powMat,encMat,timeMat,Y,sessions,folds,thisFold,sel,params,powParams,thisE,thisT,thisC);        
...
function [yProb,yTest] = run_fold_with_nested_folds(X,E,T,Y,sessions,folds,thisFold,sel,params,powParams,thisE,thisT,thisC)
...
if isempty(thisC) || isempty(thisT) || isempty(thisE)

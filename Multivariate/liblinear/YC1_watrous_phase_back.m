...
%     T         = reshape(squeeze(timeLabel),size(timeLabel,1),nFreqs*nTimes*nElecs*nItemObs);
    ...
    X = load(['/scratch/jfm2/python/YC1/RAM_YC1_',subj,'/pow.mat']);
    X = X.pow_mat(encPeriod==2,:);
    T = load(['/scratch/jfm2/python/YC1/RAM_YC1_',subj,'/time.mat']);
    T = T.time_mat+1;
...
%     X = fakeX;
    ...
    for iFold = 1:nFolds
        ...
        if all(ismember([-1,1],[res.yTest{:}]))            
            [~,~,~,aucTmp] = perfcurve([res.yTest{:}],[res.yProb{:}],1);
            fprintf('Subject %s: Fold %d of %d, %.3f running AUC.\n',subj,iFold,nFolds,aucTmp)        
        end
    end
    keyboard
...
%         model = trainFun(double(yTrain),sparse(xTrain),param);
        model = trainFun(double(yTrain),sparse(xTrain),liblin_param);

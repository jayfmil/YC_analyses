function auc = YC1_refactor_noZmean_sessCV(subj,params,saveDir)
...
try
    ...
    elseif strcmp(cvField,'session')
        if length(unique(sessions)) < 3 && strcmp(nestedCvField,'session')
            fprintf('No enough sessions inner and outer session CV for %s.\n',subj)
            return
        end
    end
    ...
    res = [];
    [res.yProbs, res.yTests, res.bestEnc, res.bestTime, res.bestC, res.aucFold] = deal(cell(1,nFolds));   
    for thisFold = 1:nFolds
        fprintf('%s: fold %d of %d.\n',subj,thisFold,nFolds)
        [yProbFold,yTestFold,eFold,tFold,cFold] = run_fold_with_nested_folds(powMat,encMat,timeMat,Y,sessions,folds,thisFold,innerSel,params,powParams);        
...
[aucs,toRemove] = run_nested_fold(X,E,T,Y,sessions,folds,params,powParams);
...
function [yProb,yTest,thisE,thisT,thisC] = run_fold_with_nested_folds(X,E,T,Y,sessions,folds,thisFold,innerSel,params,powParams)
...
if size(subFolds,1) > maxInd && strcmp(params.nestedCvField,'blocknum') && ~params.useKfold
    parfor thisFold = 1:size(subFolds,1)
        ...
        probs_fold = NaN(length(Es),length(Ts),length(Cs),sum(~obsMask));
        for ind = 1:maxInd

function [perf,AUC,subject,params] = YC1_runMulti_subj_combinedFeatures_normRespCV(subj,params,saveDir)
    ...
    for thisT = 1:length(Ts)       
        ...
        for thisEnc = 1:length(types)+1
            ...
            for cv = 1:nCV
                ...
                [thresh,yTrain] = standardize_response(Y(inds));
...
if isempty(lambda)    
    ...
    [lambda,tBest,encBest,Cs,Ts,aucs] = calcPenalty(xTrain,yTrain,T,subFolds,sessions,trialType,normType,prctileThresh);         
end
if encBest == 3
    trialType(:) = 3;
end
...
% [thresh,yTrain] = standardize_response(yTrain);
% numToRemove = sum(yTrain) - sum(~yTrain);
numToRemove = 0;
maxSamp = 100;
if numToRemove == 0
    maxSamp = 1;
...
for nSamp = 1:maxSamp
    ...
    % training set y
    [thresh,yTrain] = standardize_response(Y(trainIndsSamp));
    yTrain = double(yTrain);
    yTrain(yTrain==0) = -1;
...
% standardize 
[X,m,s] = standardize(X(:,T==bestT&trialType==encBest),sessInds);
...
function [thresh,ybool] = standardize_response(y)
thresh = median(y);
ybool  = y < thresh;

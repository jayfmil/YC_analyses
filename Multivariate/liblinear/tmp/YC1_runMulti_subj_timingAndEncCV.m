function [perf,AUC,subject,params] = YC1_runMulti_subj_timingAndEncCV(subj,params,saveDir)
...
%     end    
    ...
    trialType = [events(eventsToUse).withinItemCount];
    ...
    for iFold = 1:nFolds
        ...
        [res.yProb{iFold},...
            ...
            res.Cs{iFold},...
            res.Ts{iFold},...
            res.aucs{iFold}] = doRegFun(X,Y,T,folds,iFold,lambda,normType,sessInds,trialType,events(eventsToUse),nestedCvField,modelEachTime);
...
%     X = reshape(squeeze(powerData(:,:,bestTime,:,:)),size(powerData,1),nFreqs*nElecs*nItemObs);
    resFull = doRegFullData(X,Y,T,folds,normType,sessInds,trialType);
...
function [cBest,tBest,encBest,Cs,Ts,auc_pen] = calcPenalty(X,Y,T,folds,sessInds,trialType,normType,modelEachTime)
...
for thisC = 1:length(Cs)
    ...
    for thisT = 1:length(Ts)       
        ...
        % encoding trial 1 or 2 fold
        for thisEnc = 1:length(types)
            ...
            [~,~,~,auc_pen(thisC,thisT,thisEnc)] = perfcurve(labels,dec_values,1);
...
function [yProbs,yPreds,yTest,A,err,lambda,tBest,encBest,Cs,Ts,aucs] = doRegFun(X,Y,T,folds,iFold,lambda,normType,sessInds,trialType,events,nestedCvField,modelEachTime)
...
if isempty(lambda)    
    ...
    [lambda,tBest,encBest,Cs,Ts,aucs] = calcPenalty(xTrain,yTrainBool,T,subFolds,sessions,encType,normType,modelEachTime);         
end
...
preds = NaN(sum(~trainInds),100);
probs = NaN(sum(~trainInds),100);
...
function res = doRegFullData(X,Y,T,folds,normType,sessInds,trialType)
% train on all data. No test

% find lambda
res = [];
[lambda,bestT,encBest,Cs,Ts,aucs] = calcPenalty(X,Y,T,folds,sessInds,trialType,normType); 
res.C = lambda;
res.T = bestT;
res.Cs = Cs;
res.Ts = Ts;
...
% standardize 
[X,m,s] = standardize(X(trialType==encBest,T==bestT),sessInds(trialType==encBest));

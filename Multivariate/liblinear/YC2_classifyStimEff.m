function res = YC2_classifyStimEff(subjs)
...
saveDir = '/scratch/jfm2/YC1/multi/acrossSess/YC2_stimEff';
...
parfor s = 1:length(subjs)
    ...
    % run
    try
        resSubj = YC2_classifyStimEff_subj(subjs{s},yc1Data,saveDir);
        res = [res resSubj];
    end    
end
...
function res = YC2_classifyStimEff_subj(subj,yc1Data,saveDir)
fname = fullfile(saveDir,[subj '_yc2_stimEff.mat']);
res = [];

params = yc1Data.params;
params.timeBins = [310 400];
...
eventsToUse = evMask;
stimMask    = [events(eventsToUse).isStim]==1;
nonStimMask = ~stimMask;
if sum(eventsToUse) < 96
...
if strcmp(params.encPeriod,'first')
    ...
    nonStimMask = ~stimMask(encPeriod==1);
    stimMask    = stimMask(encPeriod==1);
elseif strcmp(params.encPeriod,'second')
    powerData   = powerData(encPeriod==2,:,:,:);
    nonStimMask = ~stimMask(encPeriod==2);    
    stimMask    = stimMask(encPeriod==2);    
end
...
for s = 1:length(uniqSess)
    ...
    stimMaskSess = stimMask(thisSess);
    nonStimMaskSess = nonStimMask(thisSess);
    resSess = [];
...
%     ySess = ySess < median(ySess);
    ...
    % classify post stim period
    [predsStim,accStim,probsStim] = predict(double(ySess(stimMaskSess)),sparse(xSess(stimMaskSess,:)),yc1Data.res.model,'-b 1 -q');
    
    % classify post non stim period
    [predsNonStim,accNonStim,probsNonStim] = predict(double(ySess(nonStimMaskSess)),sparse(xSess(nonStimMaskSess,:)),yc1Data.res.model,'-b 1 -q');
    
    ressSess.probsStim      = probsStim;
    resSess.yStim           = ySess(stimMaskSess);
    ressSess.probsNonStim   = probsNonStim;
    resSess.yNonStim        = ySess(nonStimMaskSess);
    resSess.classStimChange = mean(probsStim(:,1)) - mean(probsNonStim(:,1));
    resSess.behStimChange   = mean(resSess.yStim==1) - mean(resSess.yNonStim==1);    
    resSess.subj = subj;
    resSess.session = s    
    res = [res resSess];

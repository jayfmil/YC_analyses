function YC_classParamsWrapper_varWindowSize(subjs,enc_period,just_loso)
%
...
% base save directory
baseDir = '/scratch/jfm2/YC1/multi/gridsearch_varWindowSize';
baseDir = [baseDir '_enc_' num2str(enc_period)]
if ~doBipol
...
windowSizes  = 5:10:390;
binCenters = mean([[5:windowSizes(1):390-windowSizes(1)]' [5+windowSizes(1):windowSizes(1):390]'],2);
...
%                 params.usePhase = 3
                ...
                aucsSubj(ind) = YC1_refactor(subj,params,saveDir);                
            end
        end
        aucsAll(:,:,:,:,s) = aucsSubj;
...
for f = 1:nFreqs
    for r = 1:nRegions
        ...
        set(gca,'xtick',[1:4:nBinCenters])
        set(gca,'xticklabel',binCentersMS(1:4:end))        
        ...
        hold on
        for v = 1:length(vert_lines)
            z = find(binCentersMS > vert_lines(v),1,'first') - .5;
...
texName = 'auc_triangle_rec_vs_non_rec_bipol';
if doBipol == 0
    texName = 'auc_triangle_rec_vs_non_rec_avg_ref';
end
if just_loso

...
if doBinary
    ...
    for nSub = 1:nSubSamps
        ...
        if isempty(lambda)
            if strcmp(normType,'L1')
                [A_lasso, stats] = lassoglm(xTrain',yTrainBool,'binomial','CV', nCV, 'NumLambda', 20,'alpha',alpha);
                
                % get the best cooefficients and intercept
                As(:,nSub)       = A_lasso(:,stats.IndexMinDeviance);
                intercepts(nSub) = stats.Intercept(stats.IndexMinDeviance);
                lambda           = stats.LambdaMinDeviance;

function YC1_R1059J(subjs,params)
...
h=figure('units','pixels','position',[100 100 300 500]);
clf
whitebg(1,'k');
set(gcf,'color','k');
F(length(vals)) = struct('cdata',[],'colormap',[]);
vidObj = VideoWriter('GoodGoodNoStim.avi');
open(vidObj);
for i = 1:length(vals)
    y = vals(i)-.5;
    if y < 0;
    ...
    bar(y,c,'linewidth',3,'EdgeColor','w');
    set(gca,'ylim',[-.5 .5])
    set(gca,'xtick',[]);
    set(gca,'yticklabel','');
    drawnow
    ...
    rect = [0,5, pos(3)-10, pos(4)-10];
    ylabel('Classifer Output','Fontsize',40)
    F = getframe(gcf);        
    writeVideo(vidObj,F);
end
close(vidObj)

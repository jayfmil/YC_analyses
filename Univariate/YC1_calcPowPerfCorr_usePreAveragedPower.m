function YC1_calcPowPerfCorr_usePreAveragedPower(subjs,params)
...
try
    ...
    [powerData,fullPowerSpect,freqs] = loadAllPower(tal,subj,events,params.freqBins,eventsToUse,params);    
    powerData = permute(powerData,[2 3 1]);
    fullPowerSpect = permute(fullPowerSpect,[2 3 1]);
    ...
    locTags = {tal.locTag};
    save(fname,'res','tal','powerData','fullPowerSpect','objLocs','respLocs','startLocs','er','locTags','freqs','isInnerBand')
catch e
    fprintf('Error processing %s.\n',subj)    
end


function [powerData,powerDataUnbinned,freqs] = loadAllPower(tal,subj,events,freqBins,eventsToUse,params)
...
if ~params.averageRegion
    fieldToUse = 'zPowAll';
    fieldToUse = 'powAll';
    powerData = NaN(nFreqs,nEvents,nElecs);
else
    fieldToUse = 'powAll';
    powerData = NaN(nFreqs,nEvents);
end

for e = 1:nElecs
    elecNum = tal(e).channel;
    
    basePath = params.preAveragePowerPath;
    subjPath  = fullfile(basePath,subj);
    
    fname = fullfile(subjPath,[num2str(elecNum(1)),'-',num2str(elecNum(2)),'.mat']);
    subjData = load(fname);
    
    if length(events) ~= size(subjData.powAll,1)
        fprintf('Number of events does not match size of power matrix for %s!.\n',subj)
        return
    end
    
    if e == 1
        nFreqsAll = size(subjData.powAll,2);
        powerDataUnbinned = NaN(nFreqsAll,nEvents,nElecs);
    end
    subjPowElec = subjData.(fieldToUse)(eventsToUse,:);
    powerDataUnbinned(:,:,e) = subjPowElec';
end

% if not averaging electrodes, just average frequency
if params.averageRegion
    powerDataUnbinned = nanmean(powerDataUnbinned,3);
end
if nFreqs ~= size(subjData.powAll,2)
    for f = 1:nFreqs
        fInds = subjData.freqs >= freqBins(f,1) & subjData.freqs < freqBins(f,2);
        tmpPower = nanmean(powerDataUnbinned(fInds,:,:),1);
        
        % zscore within frequency
        for e = 1:size(tmpPower,3)
            powerData(f,:,e) = zscore_local(tmpPower(:,:,e)', subjData.sessions(eventsToUse)');
        end
    end
else
    powerData = powerDataUnbinned;
end
freqs = subjData.freqs;
%     % otherwise, average eletrodes (non zscored), then frequency
% else    
%     powerDataUnbinned = nanmean(powerDataUnbinned,3);
%     if nFreqs ~= size(subjData.powAll,2)
%         for f = 1:nFreqs
...
%             
%             % zscore within frequency
%             powerData(f,:) = zscore_local(tmpPower', subjData.sessions(eventsToUse)');
%         end
%     else
%         powerData = powerDataUnbinned;
%     end    

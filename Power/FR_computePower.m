function FR_computePower(task,subjs,params,fileExt)
...
for s = 1:length(subjs)
    ...
    try
        ...
        if params.doBipol
            ...
            tal = getBipolarSubjElecs(subj,0,0,0);
        end
        cd(params.savedir);
...
%     dsDur = ceil(size(pow,3)/params.pow.timeStep);    
    dsDur = (size(phase,3) / (resampledrate/1000))/params.pow.timeStep;
...
%     [phase,binPowMat] = getphasepow_bipol(Elec.channel,events,params.eeg.durationMS,params.eeg.offsetMS,params.eeg.bufferMS,'freqs',params.pow.freqs,'width',params.pow.wavenum,'filtfreq',params.eeg.filtfreq,'filttype',params.eeg.filttype,'filtorder',params.eeg.filtorder,'downsample',dsamp);
    [~,binPowMat] = getphasepow_bipol(Elec.channel,events,params.eeg.durationMS,params.eeg.offsetMS,params.eeg.bufferMS,'freqs',params.pow.freqs,'width',params.pow.wavenum,'filtfreq',params.eeg.filtfreq,'filttype',params.eeg.filttype,'filtorder',params.eeg.filtorder);
    binPowMat = permute(binPowMat,[2 3 1]);
    binPowMat = log10(binPowMat);
    binPowMat = nanmean(binPowMat,2);
%     phase     = permute(phase,[2 3 1]);
end


% use mean/sd across word events to zscore
if strcmp(task,'RAM_FR1')
    baseInds    = params.eventsFR1(events);
elseif strcmp(task,'RAM_TH2')
    baseInds    = params.eventsTH2(events);

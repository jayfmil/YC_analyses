...
task = 'RAM_FR1';
subjs = get_subs('RAM_FR1');
subjsTH = get_subs('RAM_TH1');
subjs = subjs(ismember(subjs,subjsTH));
% subjs = {'R1163T','R1167M'};


params.doBipol = 1;

params.eeg.durationMS   = 1600;
params.eeg.offsetMS     = 0;
...
params.regressTrialNumber = 0;

% if YC1, use all learning trials to zscore
params.eventsFR1        = @(events)strcmp({events.type},'WORD');

% compute powers
fileExt = '';
FR_computePower(task,subjs,params,fileExt)

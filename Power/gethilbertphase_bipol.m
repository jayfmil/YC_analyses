...
if ~exist('resampleFreq','var') || isempty(resampleFreq)
    ...
    [resampleFreq] = GetRateAndFormat(events(2));
end
...
if length(chan)==1
    eeg = gete_ms(chan,events,DurationMS+(2*BufferMS),OffsetMS-BufferMS,0,notchFreqRange,'stop',2,resampleFreq);
elseif length(chan)==2
    eeg1 = gete_ms(chan(1),events,DurationMS+(2*BufferMS),OffsetMS-BufferMS,0,notchFreqRange,'stop',2,resampleFreq);
    eeg2 = gete_ms(chan(2),events,DurationMS+(2*BufferMS),OffsetMS-BufferMS,0,notchFreqRange,'stop',2,resampleFreq);
    eeg = eeg2-eeg1;
    clear eeg2 eeg1
end
